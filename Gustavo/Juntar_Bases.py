...
temp = pd.merge(df1,df2,how='inner',on='CD_PROGRAMA_IES', suffixes=('', '_drop'))
temp.drop([col for col in temp.columns if 'drop' in col], axis=1, inplace=True)

df_merged=pd.merge(temp,df3,how='inner',on='CD_PROGRAMA_IES', suffixes=('', '_drop'))
...
#salvando o csv
df_merged.to_csv('data/Para_juntar/Grande Base.csv',encoding='latin1',index=False)
